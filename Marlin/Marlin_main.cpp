	...
	if(code_seen('G'))
	{
		switch((int)code_value())
		{
			...
			case 40://G40 --> X Extruder calibration
			{
				...
				while (degHotend(LEFT_EXTRUDER)<(degTargetHotend(LEFT_EXTRUDER)-10) && degHotend(RIGHT_EXTRUDER)<(degTargetHotend(RIGHT_EXTRUDER)-10)){ //Waiting to heat the extruder
				...
				for (int i=0;(i<(NUM_LINES));i++) //4 times
				{
					if (i == 0){
						...
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i),250+5, current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i),250+5, current_position[Z_AXIS], current_position[E_AXIS]-=2, max_feedrate[X_AXIS]/2 , active_extruder);
						st_synchronize();
					}
					if (i ==9){
						...
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)-1, 250+5, current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)-1, 250+5, current_position[Z_AXIS], current_position[E_AXIS]-=2, max_feedrate[X_AXIS]/2 , active_extruder);
						st_synchronize();
						//draw a zero
						...
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)+2,250+11,current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)+2,250+5, current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)+4,250+5, current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)+4,250+11, current_position[Z_AXIS], current_position[E_AXIS]+=2, max_feedrate[X_AXIS]/2 , active_extruder);
						plan_buffer_line(mm_left_offset+(mm_each_extrusion*i)+4,250+11, current_position[Z_AXIS], current_position[E_AXIS]-=2, max_feedrate[X_AXIS]/2 , active_extruder);
						st_synchronize();
					}
				...
				current_position[X_AXIS]=mm_left_offset+10*mm_each_extrusion;
				...
				for (int i=0; i<(NUM_LINES);i++) //4 times
				{
					...
					st_synchronize();
					current_position[E_AXIS]+=2;
					plan_buffer_line(mm_left_offset+(mm_second_extruder[i]+(mm_each_extrusion*(i))), 149, current_position[Z_AXIS], current_position[E_AXIS], 50 , active_extruder); //Move the retracked space
					Serial.println("We are restoring retrack");
					st_synchronize();
					current_position[E_AXIS]+=1;
					...
					current_position[E_AXIS]-=2;
					plan_buffer_line(mm_left_offset+(mm_second_extruder[i]+(mm_each_extrusion*(i))), 199, current_position[Z_AXIS], current_position[E_AXIS], 50, active_extruder);//Retrack
					//plan_buffer_line(mm_left_offset+(mm_each_extrusion*i), 150, current_position[Z_AXIS]+5, current_position[E_AXIS], 1500/60, active_extruder);//Lift Z
					st_synchronize();
			...
			case 41://G41 --> Y Extruder calibration
			{
				...
				for (int i=0; i<(NUM_LINES);i++) //4 times
				{
					if (i == 0){
						 ...
						 plan_buffer_line(100-6, mm_left_offset-(mm_each_extrusion*i)+4, current_position[Z_AXIS], current_position[E_AXIS], max_feedrate[X_AXIS]/2 , active_extruder);
						 st_synchronize();
					}
					if (i ==9){
						//draw a one						
				...
				for (int i=0; i<(NUM_LINES);i++) //4 times
				{
					...
					st_synchronize();
					current_position[E_AXIS]+=2;
					plan_buffer_line(200, mm_left_offset-(mm_second_extruder[i]+(mm_each_extrusion*(i))), current_position[Z_AXIS], current_position[E_AXIS], 50 , active_extruder);//Move the retracked space
					Serial.println("We are restoring retrack");
					st_synchronize();
					current_position[E_AXIS]+=1;
					...
					current_position[E_AXIS]-=2;
					plan_buffer_line(151, mm_left_offset-(mm_second_extruder[i]+(mm_each_extrusion*(i))), current_position[Z_AXIS], current_position[E_AXIS], 50, active_extruder);//Retrack
					//plan_buffer_line(mm_left_offset+(mm_each_extrusion*i), 150, current_position[Z_AXIS]+5, current_position[E_AXIS], 1500/60, active_extruder);//Lift Z
					st_synchronize();
				}
